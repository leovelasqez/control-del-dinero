    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 360, textAlign: 'center' }}>
        ...
        <div className="flex gap-2" style={{ justifyContent: 'center' }}>
          <button className="btn btn-ghost btn-sm" onClick={onCancel} style={{ flex: 1 }}>Cancelar</button>
          <button className="btn btn-danger btn-sm" onClick={onConfirm} style={{ flex: 1 }}>Eliminar</button>
        </div>
      </div>
    </div>
  )
